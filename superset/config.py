...
APP_ICON = "/static/assets/images/ADF_DVT_LOGO.svg"
APP_ICON_WIDTH = 170
...
KP_DEPLOYED_ENV = config('SUPERSET_DEPLOYED_ENV')
BANNER_TEXT = f"Welcome to ADF-DVT {KP_DEPLOYED_ENV} Platform. This is a private computer system. Access requires explicit current authorization and is limited to business use only. All users expressly consent to monitoring by system personnel to detect improper use or access. If such monitoring reveals possible criminal activity, or improper use of or access to the system, system personnel may provide evidence of such conduct to law enforcement officials and/or company management."
...
OAUTH_PROVIDERS = [
 {   'name':'pingid',
            ...
            'remote_app': {
                'consumer_key':'KP_DVT_ONCLOUD_DEV',
                'consumer_secret':'oBOTUiABrRq40Yo4aaeewpaeY6uAfZHot9bjxiMm3FpYyXoxUCfGNICzap7Bp6N3',
                'request_token_params':{
                    'scope': 'openid profile email'
                },
                'access_token_method':'POST',
                'base_url':'https://fam-uat.kp.org/',
                'access_token_url':'https://fam-uat.kp.org/as/token.oauth2',
                'authorize_url':'https://fam-uat.kp.org/as/authorization.oauth2'
            }
        }
]
...
class CustomProxyFix(object):
     ...
     def __call__(self, environ, start_response):
          url_scheme = environ.get('HTTP_X_FORWARDED_PROTO',None)
          host = environ.get('HTTP_X_DVT_HEADER', None)
          if url_scheme:
...
class CustomOAuthDBView(AuthOAuthView):
    def __init__(self):
        super(CustomOAuthDBView, self).__init__()
    @expose("/logout/")
    def logout(self):
        print("printing the session--------->",session)
        logout_user()
        session.pop('OAUTH', None)
        session.pop('OAUTH_PROVIDER', None)
        return redirect("https://fam-uat.kp.org/idp/startSLO.ping")

# CustomSsoSecurityManager that extends SupersetSecurityManager and overrides oauth_user_info
class PingIdSsoSecurityManager(SupersetSecurityManager):
    authoauthview = CustomOAuthDBView
    def oauth_user_info(self, provider, response=None):
        logging.debug("Oauth2 provider PingIdSsoSecurityManager: {0}.".format(provider))
        if provider == 'pingid':
            me = self.appbuilder.sm.oauth_remotes[provider].get('idp/userinfo.openid').data
            logging.debug("user_data  PingIdSsoSecurityManager: {0}".format(me))
            return { 'email' : me['cn']+'@kp.org', 'id' : me['sAMAccountName'],
                     'username' : me['sub'], 'first_name':me['Firstname'], 'last_name':me['Lastname'],'full_name':me['cn']}

CUSTOM_SECURITY_MANAGER = PingIdSsoSecurityManager
