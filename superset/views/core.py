...
class Superset(BaseSupersetView):
    ...
    @expose("/csv/<client_id>/<full_set_bool>")
    @event_logger.log_this
    def csv(self, client_id, full_set_bool='False'):
        ...
        if blob:
            ...
            #If user wants to download full results with no limits, then strip limit
            if ( full_set_bool == 'True' ):
                logging.info("Exporting CSV file: Full Download")
    ...
    @expose("/excel/<client_id>/<full_set_bool>")
    @event_logger.log_this
    def excel(self, client_id, full_set_bool='False'):
        ...
        if results_backend and query.results_key:
            logging.info(
                "Fetching Data from results backend " "[{}]".format(query.results_key)
            )
            blob = results_backend.get(query.results_key)
        if blob:
            logging.info("Decompressing")
            ...
            #NEW CODE TO EXPORT TO EXCEL
            logging.info("Using pandas to convert to Excel")

            #Setup engine to create Excel and convert dataframe to excel
            output = BytesIO()
            writer = pd.ExcelWriter(output, engine='xlsxwriter')
            df.to_excel(writer, index=False, **config.get("CSV_EXPORT"))

            #Save the excel workbook, and assign to variable to be able to export
            writer.save()
            output.seek(0)
            excel = output.read()
        else:
        ...
        response = Response(excel, mimetype="application/vnd.ms-excel")
        response.headers[
            "Content-Disposition"
        ...
        logging.info(
            f"EXCEL exported: {repr(event_info)}", extra={"superset_event": event_info}
        )
        return response
