...
class Hive2EngineSpec(PrestoEngineSpec):
    ...
    def fetch_data(cls, cursor, limit: int) -> List[Tuple]:
        import pyhive
        try:
            return super(Hive2EngineSpec, cls).fetch_data(cursor, limit)
    ...
    def adjust_database_uri(cls, uri, selected_schema=None):
        logging.debug("hive2 adjust db uri" + str(uri))
        if selected_schema:
             db = selected_schema + uri.database[uri.database.index(";"):]
             uri.database = parse.quote(db, safe="")
        return uri
